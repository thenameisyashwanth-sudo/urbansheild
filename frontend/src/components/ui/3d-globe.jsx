  ...
  return (
    <group visible={isVisible}>
      <mesh position={lineCenter} quaternion={lineQuaternion}>
        ...
        <meshBasicMaterial color={hovered ? "#00f0ff" : "#64748b"} transparent opacity={hovered ? 0.9 : 0.6} />
      </mesh>
      <mesh position={surfacePosition} quaternion={lineQuaternion}>
        <coneGeometry args={[0.015, 0.04, 8]} />
        <meshBasicMaterial color={hovered ? "#00f0ff" : "#ec4899"} />
      </mesh>
      <group ref={imageGroupRef} position={topPosition}>
        <Html transform center sprite distanceFactor={10} style={{ pointerEvents: isVisible ? "auto" : "none", opacity: isVisible ? 1 : 0, transition: "opacity 0.15s" }}>
          <div
            className={cn(
              "cursor-pointer overflow-hidden rounded-full bg-cyber-card shadow-lg transition-transform duration-200 border border-cyber-border",
              hovered && "scale-125 shadow-[0_0_15px_rgba(0,240,255,0.5)] ring-1 ring-accent"
            )}
  ...
  return (
    <group>
      <mesh geometry={geometry}>
        <meshStandardMaterial map={earthTexture} bumpMap={bumpTexture} bumpScale={config.bumpScale * 0.05} roughness={0.7} metalness={0} />
      </mesh>
      ...
      {markers.map((marker, index) => (
        <Marker key={`m-${index}-${marker.lat}-${marker.lng}`} marker={marker} radius={config.radius} defaultSize={config.markerSize} onClick={onMarkerClick} onHover={onMarkerHover} />
      ))}
    </group>
  )
}
...
function Scene({ markers, config, onMarkerClick, onMarkerHover }) {
  ...
  return (
    <>
      ...
      <directionalLight position={[-config.radius * 3, config.radius, -config.radius * 2]} intensity={config.pointLightIntensity * 0.3} color="#00f0ff" />
  ...
  return (
    <Html center>
      <div className="flex flex-col items-center gap-3">
        <span className="text-sm text-accent animate-pulse">Loading globe...</span>
      </div>
    </Html>
  )
}
